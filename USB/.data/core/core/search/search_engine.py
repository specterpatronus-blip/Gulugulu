...
class SearchEngineV2:
    ...
    def search(self, query, filters=None):
        ...
        try:
            ...
            # 3. Case: High-Boosting Search
            search_tokens = re.findall(r'\w+', query.lower())
            search_tokens = [t for t in search_tokens if len(t) > 1]
            ...
            # RANKING WEIGHTS: 
            # title (idx 1): 50.0 | subject (idx 3): 100.0 | description (idx 2): 5.0 | content (idx 5): 1.0
            sql = f'''
                SELECT 
                    lo.id, lo.pedagogical_id, lo.title, lo.description, lo.slug,
                    lo.curation_score, lo.is_verified,
                    s.name as subject_name, g.name as grade_name,
                    bm25(search_index, 0, 50.0, 5.0, 100.0, 2.0, 1.0) as text_relevance,
                ...
                ORDER BY (bm25(search_index, 0, 50.0, 5.0, 100.0, 2.0, 1.0) * -1) + (lo.curation_score * 5) DESC
                LIMIT 50
            '''
