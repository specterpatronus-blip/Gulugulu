...
(function (global) {
  ...
  const EMOJI_TO_KEY = {
    ...
    '🌵': 'cactus',
    '🐕': 'dog',
    '🏃': 'kangaroo', // Jump/run action mapping
    '👧': 'polar_bear', // Simple fallback character or custom icon
    '🦘': 'kangaroo',
    '🍎': 'tomato', // Fallback to fruit/tomato
    // Organs
    '🫃': 'stomach',
    '🫁': 'lungs',
    '❤️': 'heart',
  ...
  function buildSVG(key, opts = {}) {
    ...
    return `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="${size}" height="${size}" class="gicon ${cls}" style="display:inline-block;vertical-align:middle;flex-shrink:0;${style}" aria-label="${def.label}" role="img">${innerPaths}</svg>`;
  }
